...
- `components/` are presentational only (`ResultsView` groups checks by `group`, defaults every group's accordion to expanded, and renders `AuditSection` rows under a `ScoreCard`). Fetch/loading/error state lives in `app/page.tsx`, not in the components. `ErrorAlert` maps real HTTP status codes (400/429/500/502/504) to alert copy — it does not fabricate error content.
